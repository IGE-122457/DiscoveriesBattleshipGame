# Battleship
# Projeto: Grupo dos Fixes
Este repositório contém o desenvolvimento do jogo de estratégia naval focado na era dos Descobrimentos.

## 👥 Equipa

| Número | Nome Completo | Curso |
| :--- | :--- | :--- |
| 122457 | António Namniyek | Informática e Gestão de Empresas |
| 122478 | Gonçalo Silva | Informática e Gestão de Empresas |
| 122482 | Leonardo Teodoro | Informática e Gestão de Empresas |
| 122488 | Tiago Soeiro | Informática e Gestão de Empresas |

---

## 🚢 Tipos de Navios
No jogo, as unidades disponíveis baseiam-se na frota histórica portuguesa:

* **Nau:** Navio de grande porte, resistente e com grande capacidade de carga.
* **Caravela:** Veloz e manobrável, ideal para exploração e bolinar contra o vento.
* **Galeão:** Poderosa unidade militar, fortemente armada para combate.



---

## 📜 Regras do Jogo
1. **Posicionamento:** Cada jogador coloca a sua frota num tabuleiro de 10x10.
2. **Turnos:** Os jogadores alternam disparos indicando coordenadas (ex: A5, B2).
3. **Objetivo:** Ganha quem afundar todos os navios da frota adversária primeiro.
4. **Restrições:** Os navios não se podem sobrepor nem ficar fora dos limites do mapa.

---

## 📚 Referências Históricas
Para quem quiser saber mais sobre a tecnologia naval da época:
* [A Caravela (Wikipedia)](https://pt.wikipedia.org/wiki/Caravela)
* [As Naus da Carreira da Índia](https://pt.wikipedia.org/wiki/Nau)
Basic academic version of Battleship game to build upon.
